...
func add_play_test_scene_button()->void:
	...
	var play_test_button_scene := load("uid://dbjvqwv7nf7e") as PackedScene
...
func search_play_current_button(node:Node)->Button:
	if node is Button and (node as Button).name == "@Button@4775":
		# Alternative: button.tooltip_text == "Play the currently edited scene."
